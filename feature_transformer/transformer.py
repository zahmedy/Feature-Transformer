...
class FeatureTransformer(BaseEstimator, TransformerMixin):
    ...
    def _compute_feature_names_out(self) -> np.ndarray:
        """Compute feature names after preprocessing (and feature selection if used)."""
        preprocessor: ColumnTransformer = self._pipeline.named_steps["preprocessor"]

        # get names from ColumnTransformer output
        base_feature_names = preprocessor.get_feature_names_out()

        # apply feature selection mask if used
        if self.k_best is not None and "select" in self._pipeline.named_steps:
            selector: SelectKBest = self._pipeline.named_steps["select"]
            support_mask = selector.get_support()
            base_feature_names = base_feature_names[support_mask]
